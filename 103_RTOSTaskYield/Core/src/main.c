...
#define TRUE 1
#define FALSE 0
#define AVAILABLE_TO_RUN TRUE
#define NOT_AVAILABLE_TO_RUN FALSE

/*Global variables section*/
char userText[99];
uint16_t uartAccessKey = AVAILABLE_TO_RUN;
...
int main(void) {
  ...
  xTaskCreate(vTask1_Function, "Task1", configMINIMAL_STACK_SIZE, NULL, 2,
              &xTask1_Handler);

  xTaskCreate(vTask2_Function, "Task2", configMINIMAL_STACK_SIZE, NULL, 2,
              &xTask2_Handler);
...
#endif
    if (uartAccessKey == AVAILABLE_TO_RUN) {
      uartAccessKey = NOT_AVAILABLE_TO_RUN;
      USART_SendText(USART3, "Task 1 function\r\n");
      uartAccessKey = AVAILABLE_TO_RUN;
      /*RTOS utility: Macro for forcing a context switch*/
      taskYIELD();
    }
  }
}
...
static void prvSetupHardware(void) { prvSetupUart(); }
