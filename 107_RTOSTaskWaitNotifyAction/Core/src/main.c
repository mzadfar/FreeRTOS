...
int main(void) {
  ...
  xTaskCreate(PushButton_task_Function, "PushButton_task", 500, NULL, 2,
              &xTask2_Handler);
...
void LED_task_Function(void *funcParams) {
  uint32_t currentNotificatonValue = 0;
  while (1) {
    /*Wait until any notification event from button task received*/
    if (xTaskNotifyWait(0, 0, &currentNotificatonValue, portMAX_DELAY) ==
        pdTRUE) {
      GPIO_ToggleBits(GPIOB, GPIO_Pin_0 | GPIO_Pin_7 | GPIO_Pin_14);
      USART_SendText(USART3,
                     "Notification is received, Push Button press count:\t");
      USART_SendNumber(USART3, currentNotificatonValue);
      USART_SendText(USART3, "\n");
    }
  }
}
...
void PushButton_task_Function(void *funcParams) {
  while (1) {
    if (GPIO_ReadInputDataBit(GPIOC, GPIO_Pin_13)) {
      rtosDelayMs(100);
      xTaskNotify(xTask1_Handler, 0x0, eIncrement);
