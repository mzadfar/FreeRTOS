...
void vTask4_Function(void* funcParams); // Push button
...
int main(void)
{
    ...
    status = xTaskCreate(
        vTask4_Function, "Task4", configMINIMAL_STACK_SIZE, NULL, 4, &xTask4_Handler);
    configASSERT(status == pdPASS);
...
void vTask4_Function(void* funcParams)
{
    uint8_t buttonRead = 0;
    uint8_t prevButtonRead = 0;
    while (1)
    {
        buttonRead = GPIO_ReadInputDataBit(GPIOC, GPIO_Pin_13);
